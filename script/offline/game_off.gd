...
func submit_move() -> void:
	...
	var valid_words := []
	for w in formed:
		if dict.words.has(w.word) and !placed_words.has(w.word) and !opponent_words.has(w.word):
			for inc in w.cells:
	...
	for w in valid_words:
		if first_move:
			...
			if opponent_words.has(w):
				continue
			opponent_words.append(w)
	print(placed_words+opponent_words)
	if valid_words.size() <= 0:
		placed_this_turn.clear()
	if first_move:
		score = ("".join(placed_words)).length()
		$player/status/score.text = "(" + str(score) + ")"
	else:
		opponent_score = ("".join(opponent_words)).length()
		$opponent/opponent_status/score.text = "(" + str(opponent_score) + ")"
